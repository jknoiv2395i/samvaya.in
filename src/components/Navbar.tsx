...
export const Navbar: React.FC<NavbarProps> = ({ onOpenEarlyAccess }) => {
  ...
  useEffect(() => {
    const ids = ["features", "voice", "pricing"]
  ...
  return (
    <header className="w-full pt-3 md:pt-4 px-3 sm:px-4 sticky top-0 z-50">
      <div className="max-w-5xl mx-auto bg-white/70 sm:bg-white/20 backdrop-blur-xl border border-white/40 sm:border-white/30 rounded-full px-3 xs:px-4 sm:px-5 py-1.5 flex items-center justify-between shadow-[0_2px_20px_-4px_rgba(0,0,0,0.08)] transition-all">
        {/* Brand Logo */}
        <a 
          href="#" 
          onClick={(e) => { 
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="md:hidden w-10 h-10 xs:w-11 xs:h-11 rounded-[14px] xs:rounded-[16px] bg-white border border-neutral-300/80 text-neutral-900 flex items-center justify-center hover:bg-neutral-50 active:scale-95 transition-all shadow-xs shrink-0 cursor-pointer"
          >
            {isMobileMenuOpen ? (
      ...
      {/* Mobile Card Menu Overlay - styled exactly as requested */}
      {isMobileMenuOpen && (
